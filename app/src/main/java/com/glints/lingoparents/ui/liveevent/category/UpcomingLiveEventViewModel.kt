...
class UpcomingLiveEventViewModel(private val tokenPreferences: TokenPreferences) : ViewModel() {
    companion object {
    ...
    private fun onApiCallError(message: String) = viewModelScope.launch {
        upcomingLiveEventListChannel.send(
            UpcomingLiveEventListEvent.Error(
                message
            )
        )
    }
    ...
    fun onUpcomingLiveEventItemClick(id: Int) = viewModelScope.launch {
        upcomingLiveEventListChannel.send(
            UpcomingLiveEventListEvent.NavigateToDetailLiveEventFragment(
                id
            )
        )
    }

    fun loadUpcomingLiveEventList() = viewModelScope.launch {
        onApiCallStarted()
        APIClient
            .service
            .getLiveEventsByStatus(mapOf("status" to UPCOMING_TYPE))
    ...
    fun searchUpcomingLiveEventList(title: String) = viewModelScope.launch {
        ...
        APIClient
            .service
            .getUpcomingLiveEventByStatusAndTitle(title)
